 ...
 * A box is checked when the corresponding artifact exists
...
const hasPlay  = exists('playwright-summary.json') || exists('playwright-summary-pr.json');
const hasLint  = exists('lint-summary.json');
const hasFlow  = exists('flowchart.png') || exists('flowchart.mmd');
const hasBadge = exists('test-summary.txt') || hasPlay;
...
const lines = [
  ...
  `- [${hasBadge ? 'x' : ' '}] Test summary generated`,
